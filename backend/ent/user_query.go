...
type UserQuery struct {
	...
	// eager-loading edges.
	withUSERRECORD *RecordfoodQuery
...
// QueryUSERRECORD chains the current query on the USERRECORD edge.
func (uq *UserQuery) QueryUSERRECORD() *RecordfoodQuery {
	query := &RecordfoodQuery{config: uq.config}
...
//  WithUSERRECORD tells the query-builder to eager-loads the nodes that are connected to
// the "USERRECORD" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUSERRECORD(opts ...func(*RecordfoodQuery)) *UserQuery {
	query := &RecordfoodQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUSERRECORD = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUSERRECORD; query != nil {
		...
		query.Where(predicate.Recordfood(func(s *sql.Selector) {
			s.Where(sql.InValues(user.USERRECORDColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_userrecord
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_userrecord" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_userrecord" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.USERRECORD = append(node.Edges.USERRECORD, n)
		}
	}
