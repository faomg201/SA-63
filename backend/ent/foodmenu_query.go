...
// QueryFOODMENURECORD chains the current query on the FOODMENURECORD edge.
...
// the "FOODMENURECORD" edge. The optional arguments used to configure the query builder of the edge.
...
func (fq *FOODMENUQuery) sqlAll(ctx context.Context) ([]*FOODMENU, error) {
	...
	if query := fq.withFOODMENURECORD; query != nil {
		...
		for _, n := range neighbors {
			fk := n.foodmenu_foodmenurecord
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "foodmenu_foodmenurecord" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "foodmenu_foodmenurecord" returned %v for node %v`, *fk, n.ID)
			}
